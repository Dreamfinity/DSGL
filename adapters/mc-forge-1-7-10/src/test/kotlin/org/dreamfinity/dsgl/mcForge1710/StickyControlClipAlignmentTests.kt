...
class StickyControlClipAlignmentTests {
    ...
    fun `non-sticky text input keeps draw and clip aligned`() {
        val fixture = createControlFixture(
            sticky = false,
            scrollY = 0,
    ...
    fun `sticky-clamped closed select keeps label text and clip aligned`() {
        val fixture = createControlFixture(
            ...
            controlFactory = {
                SelectNode(
                    model = selectModel(id = "sticky-select-model") {
                        option("a", "Alpha")
                        option("b", "Beta")
                    },
    ...
    fun `nested clip path stays coherent for sticky-clamped text input`() {
        val root = ContainerNode(key = "nested-clip-root").apply {
            width = 220
            height = 86
            overflowY = Overflow.Auto
        }
        ContainerNode(key = "nested-clip-top-spacer").apply {
            width = 220
            height = 22
        }.applyParent(root)
        val stickyRow = ContainerNode(key = "nested-clip-sticky-row").apply {
            width = 220
            height = 30
        ...
        val nestedClip = ContainerNode(key = "nested-clip-wrapper").apply {
            width = 180
            height = 20
            overflowX = Overflow.Hidden
            overflowY = Overflow.Hidden
        }.applyParent(stickyRow)
        val input = TextInputNode(text = "nested", key = "nested-clip-input").apply {
            width = 140
            height = 18
        }.applyParent(nestedClip)
        ContainerNode(key = "nested-clip-filler").apply {
            width = 220
            height = 280
        }.applyParent(root)
    ...
    private fun createControlFixture(
        sticky: Boolean,
        scrollY: Int,
        controlFactory: () -> DOMNode
    ): ControlFixture {
        val root = ContainerNode(key = "clip-align-root").apply {
            width = 220
            height = 86
            overflowY = Overflow.Auto
        }
        if (sticky) {
            ContainerNode(key = "clip-align-top-spacer").apply {
                width = 220
                height = 22
            }.applyParent(root)
        }
        val host = ContainerNode(key = "clip-align-host").apply {
            width = 220
            height = 28
            if (sticky) {
                inlineStyleDeclarations = styleDeclarations(
        ...
        val control = controlFactory().apply {
            width = 140
            height = 18
        }.applyParent(host)
        ContainerNode(key = "clip-align-filler").apply {
            width = 220
            height = 280
        }.applyParent(root)
    ...
    private fun assertCaretInsideActiveClip(observations: CommandObservations, nearText: String) {
        ...
        val caret = observations.rects.firstOrNull { rect ->
            rect.rawWidth == 1 &&
    ...
    private fun visibleRect(node: DOMNode): Rect {
        ...
        val w = kotlin.math.ceil((maxX - minX).toDouble()).toInt().coerceAtLeast(0)
        val h = kotlin.math.ceil((maxY - minY).toDouble()).toInt().coerceAtLeast(0)
        return Rect(x, y, w, h)
    }
    ...
    private fun floorToInt(value: Float): Int = kotlin.math.floor(value.toDouble()).toInt()
