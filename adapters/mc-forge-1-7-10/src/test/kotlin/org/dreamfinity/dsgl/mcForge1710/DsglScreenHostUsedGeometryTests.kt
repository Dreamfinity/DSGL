...
class DsglScreenHostUsedGeometryTests {
        ...
        val window =
            object : DsglWindow() {
                ...
                override fun render(): DomTree {
                    ...
                    ButtonNode(text = "btn-$generation", key = "btn-$generation")
                        .apply {
                            onMouseClick = {}
                        }.applyParent(root)
                    return DomTree(root)
    ...
    private fun hoverTarget(host: DsglScreenHost): DOMNode? = host.debugHoverTargetForTests()

    private fun resolvePointerDownTarget(host: DsglScreenHost): DOMNode? = host.debugResolvePointerDownTargetForTests()

    private fun resolveClickTarget(host: DsglScreenHost): DOMNode? = host.debugResolveClickTargetForTests()
    ...
    private fun createAbsoluteOutsideAncestorFixture(): AbsoluteOutsideAncestorFixture {
        ...
        val child =
            ButtonNode("abs-child", key = "abs-child")
                .apply {
                    width = 36
                    height = 16
    ...
    private fun createPositionedOverlapFixture(): PositionedOverlapFixture {
        ...
        val early =
            ContainerNode(key = "early", stackLayout = true)
                .apply {
                    width = 120
                    height = 60
                }.applyParent(root)
        ContainerNode(key = "later-container", stackLayout = true)
            .apply {
                width = 120
                height = 60
            }.apply {
                ButtonNode("later", key = "later")
                    .apply {
                        width = 72
                        height = 24
                    }.applyParent(this)
