...
private const val SAMPLE_PARAGRAPH =
    "MSDF/MTSDF text rendering demo in DSGL. This paragraph should wrap cleanly " +
        "in a fixed-width panel and respect font switches, opacity, and size."
...
fun UiScope.msdfFontsSection(onInfo: (String) -> Unit) {
    ...
    div({
        ...
        text("MSDF Fonts")
        text(
            "All DSGL DrawText commands go through MSDF/MTSDF rendering. Switch " +
                "font/size/color/opacity and verify wrapping.",
        ...
        text(
            "fontId=${selectedFont.fontId} " +
                "source=${selectedFont.source.name.lowercase()} " +
                "fontSize=$fontSize " +
                "opacity=$textOpacity " +
                "panelWidth=$panelWidthPercent% " +
                "formatting=${formattingMode.name.lowercase()} guides=$msdfShowBaselineGuides",
            { style = { this.color = DEMO_MUTED } },
        )
        text(
            "Drop external font packages into <gameDir>/dsgl/fonts/<subdir>/<name>.ttf + " +
                "-meta.json + -mtsdf.png and restart.",
            {
                style = {
                    color = DEMO_MUTED
