...
package org.dreamfinity.dsgl.mcForge1122.demo.sections
...
import org.dreamfinity.dsgl.mcForge1122.demo.support.DEMO_MUTED
...
fun UiScope.contextMenuSection(onInfo: (String) -> Unit) {
    ...
    fun contextMenuVisibleFiles(): List<ContextMenuDemoFile> {
        return sortedChildrenForDirectory(
            files = state.files,
    ...
    fun contextMenuDeleteFile(file: ContextMenuDemoFile) {
        ...
        if (nextSelection != "none" && sortedChildrenForDirectory(
                files = nextFiles,
    ...
    fun buildBackgroundMenu() = contextMenu(id = "demo.context.background") {
    ...
    fun buildEntryMenu(file: ContextMenuDemoFile) = contextMenu(id = "demo.context.entry.${file.id}") {
        if (file.isDirectory) {
            ...
            val listNode = div({
                ...
                if (entries.isEmpty()) {
                    div({ style = { padding = 2.px } }) {
                        text("Folder is empty. Right-click to create file/folder.", { style = { color = DEMO_MUTED } })
                    }
                } else {
...
private fun iconFor(file: ContextMenuDemoFile): String {
...
private fun collectSubtree(
    rootId: String,
    byId: Map<String, ContextMenuDemoFile>
): List<ContextMenuDemoFile> {
...
private fun collectSubtreeIds(files: List<ContextMenuDemoFile>, rootId: String): Set<String> {
    return collectSubtree(rootId, files.associateBy { it.id }).map { it.id }.toSet()
}

private fun isDescendantDirectory(
    files: List<ContextMenuDemoFile>,
    ancestorId: String,
    candidateId: String
): Boolean {
