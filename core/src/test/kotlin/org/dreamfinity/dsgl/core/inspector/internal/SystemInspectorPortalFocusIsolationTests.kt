...
class SystemInspectorPortalFocusIsolationTests {
    private val ctx =
    ...
    fun `text input keeps focus while inspector is mounted and rendered`() {
        ...
        val router = SurfaceDomInputRouter { appRoot }

        val (inspector, portalNode, inspectedRoot) = createMountedInspector()
        renderInspectorFrame(portalNode, inspectedRoot, 1L)
        ...
        repeat(4) { frame ->
            renderInspectorFrame(portalNode, inspectedRoot, 2L + frame)
            assertTrue(FocusManager.isFocused(input))
        }
    ...
    fun `text selection drag and keyboard edit work while inspector is mounted`() {
        ...
        val (_, portalNode, inspectedRoot) = createMountedInspector()
        renderInspectorFrame(portalNode, inspectedRoot, 1L)

        assertTrue(router.handleMouseDown(28, 30, MouseButton.LEFT))
        assertTrue(router.handleMouseMove(70, 30))
        renderInspectorFrame(portalNode, inspectedRoot, 2L)
    ...
    fun `range drag works while inspector is mounted`() {
        ...
        renderInspectorFrame(portalNode, inspectedRoot, 3L)
    ...
    fun `inspector still handles its own input interactions after focus isolation fix`() {
        ...
        val pickRect = inspector.portalPickToggleBounds() ?: error("pick toggle missing")
    ...
    private fun createMountedInspector(): Triple<InspectorController, SystemInspectorPortalNode, DOMNode> {
        val inspector = InspectorController()
        inspector.toggle()
        inspector.setPickMode(false)
        val portalNode = SystemInspectorPortalNode(inspector)
        val inspectedRoot = inspectedRoot()
        return Triple(inspector, portalNode, inspectedRoot)
    }

    private fun renderInspectorFrame(node: SystemInspectorPortalNode, inspectedRoot: DOMNode, revision: Long) {
