...
class InspectorDragScrollDomMigrationTests {
    ...
    fun `inspector panel drag is dom-first and controller drag authority stays demoted`() {
        ...
        val before = fixture.inspector.floatingPanelRect() ?: error("expected panel rect")
        ...
        assertTrue(
            fixture.host
                .debugEntryState(SystemPortalEntryId.Inspector)
                ?.dragSession
                ?.active == true,
        )
        ...
        val moved = fixture.inspector.floatingPanelRect() ?: error("expected moved panel rect")
    ...
    fun `inspector panel drag stays monotonic when sync cursor lags behind dom drag updates`() {
        ...
        val afterDomDrag = fixture.inspector.floatingPanelRect() ?: error("expected moved panel rect")
        assertTrue(
        ...
        syncAndRender(fixture, downX, downY)
        val afterStaleSync = fixture.inspector.floatingPanelRect() ?: error("expected panel rect after stale sync")
        ...
        val afterNextMove = fixture.inspector.floatingPanelRect() ?: error("expected panel rect after next drag move")
        assertTrue(
    ...
    fun `inspector wheel body scroll is dom-first and not controller-authoritative`() {
        ...
        val contentRect = fixture.inspector.portalContentRect()
    ...
    fun `inspector scrollbar thumb drag is dom-first and not controller-authoritative`() {
        ...
        val thumb = fixture.inspector.portalScrollbarThumbRect()
    ...
    fun `controller dragMode remains inactive during dom drag and scroll interactions`() {
        ...
        val panelRect = fixture.inspector.floatingPanelRect() ?: error("expected panel rect")
    ...
    fun `inspector color edit integration remains system-owned after drag-scroll migration`() {
        ...
        assertEquals(ScreenDomainId.System, fixture.host.debugSystemColorPickerPopupOwnerDomain())
        assertNotNull(fixture.host.debugEntryNode(SystemPortalEntryId.ColorPickerPopup))
    }
    ...
    private fun findVisibleSelectRow(fixture: Fixture): InspectorStyleEditorRowSnapshot {
        repeat(120) {
            val rows =
                fixture.inspector.portalStyleEditorRows().filter { row ->
    ...
    private fun findVisibleInputNode(fixture: Fixture, propertyKey: String): TextInputNode {
        val inspectorNode =
            fixture.host.debugEntryNode(SystemPortalEntryId.Inspector)
