...
class SystemPortalInspectorNativeEntryTests {
    private val ctx =
    ...
    @Test
    fun `inspector migration removes intermediate native portal model classes`() {
        val loadResult =
            runCatching {
                Class.forName(removedLegacyNativeModelClassName())
            }
        assertTrue(loadResult.isFailure)
    }

    private fun removedLegacyNativeModelClassName(): String = "org.dreamfinity.dsgl.core.inspector.InspectorNative" + "Over" + "layModel"

    @Test
    fun `inspector controller no longer exposes manual append portal commands path`() {
        val methodNames =
            InspectorController::class.java.methods
                .map { it.name }
        assertFalse(methodNames.contains("appendPortalCommands"))
    }

    @Test
    fun `inspector portal rebuild does not leak event bus registrations`() {
        ...
        repeat(24) { frame ->
            portalNode.bindInspectedTree(root, layoutRevision = 2L + frame)
            portalNode.updateCursor(mouseX = 984, mouseY = 144, pointerCaptured = false)
            portalNode.render(ctx, 0, 0, 1280, 720)
        }
    ...
    @Test
    fun `live inspector path is native system portal entry and anti-legacy guarded`() {
        ...
        val node = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector entry missing")
    ...
    fun `expanded inspector paints occluder above full highlight geometry`() {
        ...
        val panelRect = inspector.floatingPanelRect() ?: error("panel rect missing")
        val highlight = inspector.portalSelectedHighlight() ?: error("selected highlight missing")
        assertTrue(intersects(highlight.contentRect, panelRect))

        val inspectorNode = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector node missing")
        val directChildren = inspectorNode.children.toList()

        val occluder =
            directChildren.firstOrNull { it.key == "dsgl-system-inspector-panel-occluder" }
                ?: error("occluder node missing")
        val selectedContentFill =
            directChildren.firstOrNull { it.key == "dsgl-system-inspector-selected-content-fill" }
    ...
    fun `inspector runtime interaction path supports selection controls and system-owned color edit`() {
        ...
        val pickToggle = inspector.portalPickToggleBounds() ?: error("pick toggle missing")
        ...
        val colorAction = inspector.portalColorPickerActionBounds(StyleProperty.BACKGROUND_COLOR)
        ...
        assertEquals(ScreenDomainId.System, host.debugSystemColorPickerPopupOwnerDomain())

        val pickerNode = host.debugEntryNode(SystemPortalEntryId.ColorPickerPopup) ?: error("picker node missing")
    ...
    fun `inspector minimize restore and close reopen remain stable`() {
        ...
        val initialNode = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector node missing")
        val minimizeRect = inspector.portalMinimizeBounds() ?: error("minimize bounds missing")
        ...
        host.render(ctx, 1280, 720)
        val reopenedNode = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector node missing")
    ...
    fun `minimized inspector hides expanded panel host and keeps chip visible`() {
        ...
        val inspectorNode = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector node missing")
        val panelHostNode =
            collectNodes(inspectorNode).firstOrNull { node ->
                (node.key?.toString() ?: "").startsWith("dsgl-floating-panel-")
            } ?: error("panel host node missing")
        val minimizedChipNode =
            collectNodes(inspectorNode).firstOrNull { it.key == "dsgl-system-inspector-chip" }
    ...
    fun `inspector native path preserves scroll and scrollbar drag behavior`() {
        ...
        val contentRect = inspector.portalContentRect()
        ...
        val thumb = inspector.portalScrollbarThumbRect()
    ...
    fun `scrollbar drag release outside inspector consumes mouse up and stops capture`() {
        ...
        val outsidePoint =
            candidatePoints.firstOrNull { (x, y) -> !panelRect.contains(x, y) }
    ...
    fun `inspector-opened system color picker top controls expose hover feedback`() {
        ...
        hoverTargets.forEach { (key, rect) ->
            ...
            val pickerNode =
                host.debugEntryNode(SystemPortalEntryId.ColorPickerPopup)
                    ?: error("color picker entry missing")
            val buttonNode =
                collectNodes(pickerNode)
    ...
    fun `inspector-opened system color picker mode dropdown options hover and click reliably`() {
        ...
        assertTrue(host.debugMountedEntryIds().contains(SystemPortalEntryId.ColorPickerTransient))

        val expandedLayout = host.debugSystemColorPickerBodyLayout() ?: error("expanded color picker layout missing")
        val hslOption =
            expandedLayout.modeOptions.firstOrNull { it.mode == ColorFormatMode.HSL }
        ...
        val style = ColorPickerStyle()
        val transientNode =
            host.debugEntryNode(SystemPortalEntryId.ColorPickerTransient)
                ?: error("transient entry missing")
        val optionNode =
            collectNodes(transientNode)
        ...
        assertFalse(host.debugMountedEntryIds().contains(SystemPortalEntryId.ColorPickerTransient))
        assertTrue(host.isSystemColorPickerOpen())
        assertEquals("target", inspector.selectedKey)
    ...
    fun `inspector native body content remains clipped in narrow viewport`() {
        ...
        val bodyRect = inspector.portalContentRect()
        val inspectorNode = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector node missing")
        val bodyNode =
        ...
        val bodyLines =
            collectNodes(inspectorNode).filter { node ->
        ...
        val edgeIntersecting =
            bodyLines.filter { node ->
    ...
    fun `inspector clipped body blocks hidden row input and accepts visible portion`() {
        ...
        repeat(24) {
            ...
            edgeNode =
                interactiveNodes.firstOrNull { node ->
                    intersects(node.bounds, bodyRect) &&
                        !containsFully(
                            bodyRect,
                            node.bounds,
                        )
    ...
    fun `inspector body consumes generic scroll viewport and content state`() {
        ...
        val inspectorNode = host.debugEntryNode(SystemPortalEntryId.Inspector) ?: error("inspector node missing")
        val bodyNode =
            collectNodes(inspectorNode).firstOrNull { it.key == "dsgl-system-inspector-body" }
                ?: error("inspector body node missing")
    ...
    fun `inspector wheel scrolling works when hovering interactive input`() {
        ...
        val interactiveNode =
            allNodes.firstOrNull { node ->
                ...
                val interactiveControl =
                    key.startsWith("dsgl-system-inspector-editor-input-") ||
        ...
        val wheelNode =
            interactiveNode ?: allNodes.firstOrNull { node ->
                val probeX = node.bounds.x + 2
                val probeY = node.bounds.y + (node.bounds.height / 2).coerceAtLeast(1)
                bodyRect.contains(probeX, probeY)
    ...
    private fun inspectedRoot(): ContainerNode {
        ...
        StyleEngine
            .setInspectorOverrideLiteral(root.children.first(), StyleProperty.BACKGROUND_COLOR, "#FF112233")
            .getOrThrow()
        return root
    }
    ...
    fun `inspector consumer thumb drag remains smooth and stable on release`() {
        ...
        var previousThumbY = inspector.portalScrollbarThumbRect().y
        ...
        repeat(6) { step ->
            ...
            val currentThumbY = inspector.portalScrollbarThumbRect().y
            assertTrue(
        ...
        val settledThumbY = inspector.portalScrollbarThumbRect().y
        ...
        repeat(6) { idx ->
            ...
            assertEquals(settledThumbY, inspector.portalScrollbarThumbRect().y)
        }
    }
