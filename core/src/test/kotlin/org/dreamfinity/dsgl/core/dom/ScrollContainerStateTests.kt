...
class ScrollContainerStateTests {
    ...
    fun `generic viewport rect is exposed for scroll-capable container`() {
        ...
        val container =
            ContainerNode(padding = 4, key = "viewport")
                .apply {
                    width = 100
                    height = 60
    ...
    fun `generic content extent is distinct from viewport when content exceeds bounds`() {
        val root = ContainerNode(key = "root")
        val container =
            ContainerNode(key = "extent")
                .apply {
                    width = 80
                    height = 40
                    overflow = Overflow.Hidden
                }.applyParent(root)
        ContainerNode(key = "child-a")
            .apply {
                width = 70
                height = 30
            }.applyParent(container)
        ContainerNode(key = "child-b")
            .apply {
                width = 70
                height = 30
            }.applyParent(container)
    ...
    fun `generic scroll offsets are meaningful for scroll-capable axes`() {
        val root = ContainerNode(key = "root")
        val container =
            ContainerNode(key = "scroll-state")
                .apply {
                    width = 90
                    height = 40
    ...
    fun `overflow x auto reserves horizontal gutter when measured content width exceeds viewport`() {
        val root = ContainerNode(key = "root")
        val container =
            ContainerNode(key = "auto-horizontal")
                .apply {
                    width = 794
                    height = 634
                    overflowX = Overflow.Auto
    ...
    fun `visible and hidden overflow never expose visible scrollbars or gutters`() {
        val root = ContainerNode(key = "root")
        val container =
            ContainerNode(key = "hidden-visible-axis")
                .apply {
                    width = 100
                    height = 60
                    overflowX = Overflow.Visible
    ...
    fun `mixed overflow axis combinations resolve deterministic state`() {
        ...
        cases.forEachIndexed { index, (overflowX, overflowY, expectedScrollContainer) ->
            ...
            val expectedVisibleX =
                overflowX == Overflow.Scroll ||
                    (overflowX == Overflow.Auto && state.contentExtent.width > state.viewportRect.width)
            val expectedVisibleY =
                overflowY == Overflow.Scroll ||
                    (overflowY == Overflow.Auto && state.contentExtent.height > state.viewportRect.height)
    ...
    fun `final viewport resolution remains stable after gutter resolution`() {
        val root = ContainerNode(key = "root")
        val container =
            ContainerNode(key = "stable-viewport")
                .apply {
                    width = 120
                    height = 72
                    overflowX = Overflow.Auto
