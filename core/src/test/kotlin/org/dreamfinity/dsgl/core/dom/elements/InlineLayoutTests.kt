...
class InlineLayoutTests {
    ...
    fun `inline child measured with parent width constraint expands height after wrap`() {
        ...
        assertEquals(
            ...
            "Inline height must include wrapped second line with line-box reservation.",
        )
        assertEquals(
            2,
            inline.children
                ...
                .distinct()
                .size,
            "Inline items should wrap to two lines.",
        )
    ...
    fun `inline flow with nested flex and block items keeps content inside measured bounds`() {
        val ctx = testMeasureContext()
        val root =
            ContainerNode(key = "root.nested").apply {
                display = Display.Block
                width = 36
            }
        val inline =
            ContainerNode(key = "inline.nested")
                .apply {
                    display = Display.Inline
                    gap = 3
    ...
    fun `inline wrappers with mixed text plus nested containers do not overlap between lines`() {
        val ctx = testMeasureContext()
        val root =
            ContainerNode(key = "root.mixed").apply {
                display = Display.Block
                width = 44
            }
        val inline =
            ContainerNode(key = "inline.mixed")
                .apply {
                    display = Display.Inline
                    gap = 2
                    padding = Insets.all(2)
    ...
    fun `display section mixed inline children keep line heights stable without overlap`() {
        ...
        inline.children
            ...
            .forEach { assertContainerChildrenFit(it) }
    }
    ...
    fun `inline container explicit width larger than parent is constrained without overlap`() {
        val ctx = testMeasureContext()
        val root =
            ContainerNode(key = "root.small").apply {
                display = Display.Block
                width = 60
            }
        val inline =
            ContainerNode(key = "inline.explicit.large")
                .apply {
                    display = Display.Inline
                    width = 96
                    gap = 2
    ...
    private fun createInlineFlexItem(key: String, aWidth: Int, bWidth: Int): ContainerNode {
        val wrapper =
            ContainerNode(key = "$key.wrapper").apply {
                display = Display.Inline
                margin = Insets(1, 2, 1, 1)
                padding = Insets.all(1)
                border = Border.all(1, 0xFF000000.toInt())
        ...
        ContainerNode(key = "$key.a")
            .apply {
                width = aWidth
                height = 4
            }.applyParent(flex)
        ContainerNode(key = "$key.b")
            .apply {
                width = bWidth
                height = 4
            }.applyParent(flex)
    ...
    private fun createInlineBlockItem(key: String, topWidth: Int, bottomWidth: Int): ContainerNode {
        ...
        val block =
            ContainerNode(key = "$key.block")
                .apply {
                    display = Display.Block
                    gap = 1
                    padding = Insets.all(1)
                }.applyParent(wrapper)
        ContainerNode(key = "$key.top")
            .apply {
                width = topWidth
                height = 3
            }.applyParent(block)
        ContainerNode(key = "$key.bottom")
            .apply {
                width = bottomWidth
                height = 3
            }.applyParent(block)
    ...
    private fun createMixedInlineWrapper(key: String, includeFlex: Boolean): ContainerNode {
        val wrapper =
            ContainerNode(key = "$key.mixed.wrapper").apply {
                display = Display.Inline
                width = 24
                margin = Insets(1, 2, 1, 1)
                padding = Insets.all(1)
        ...
        if (includeFlex) {
            ...
            ContainerNode(key = "$key.flex.a")
                .apply {
                    width = 9
                    height = 4
                }.applyParent(flex)
            ContainerNode(key = "$key.flex.b")
                .apply {
                    width = 8
                    height = 4
                }.applyParent(flex)
    ...
    private fun createDisplayInlineScenario(rootWidth: Int, inlineContentWidth: Int): Pair<ContainerNode, ContainerNode> {
        val root =
            ContainerNode(key = "root.display.inline").apply {
                display = Display.Block
                width = rootWidth
            }
        val inline =
            ContainerNode(key = "display.inline.container")
                .apply {
                    display = Display.Inline
        ...
        ContainerNode(key = "display.inline.flex.item")
            .apply {
                display = Display.Inline
                margin = Insets(1, 2, 1, 1)
                padding = Insets.all(2)
            ...
            .also { wrapper ->
                ...
                ContainerNode(key = "display.inline.flex.container")
                    .apply {
                        display = Display.Flex
                        flexDirection = FlexDirection.Row
                        gap = 1
                    ...
                    .also { flex ->
                        ContainerNode(key = "display.inline.flex.a")
                            .apply {
                                width = 8
                                height = 4
                            }.applyParent(flex)
                        ContainerNode(key = "display.inline.flex.b")
                            .apply {
                                width = 6
                                height = 4
                            }.applyParent(flex)
        ...
        ContainerNode(key = "display.inline.block.item")
            .apply {
                display = Display.Inline
                margin = Insets(1, 2, 1, 1)
                padding = Insets.all(2)
            ...
            .also { wrapper ->
                ContainerNode(key = "display.inline.block.container")
                    .apply {
                        display = Display.Block
                        gap = 1
                        padding = Insets.all(1)
    ...
    private fun assertContainerChildrenFit(container: ContainerNode) {
        ...
        val usedBottom =
            container.children.maxOf { child ->
