...
class ApplicationPortalRootNode(
    key: Any? = "dsgl-application-portal-root",
) : DOMNode(key) {
    override val styleType: String = "dsgl-application-portal-root"
    private var viewportWidth: Int = 0
    private var viewportHeight: Int = 0
    private val debugTintNode: ContainerNode =
        UiScope(this).div({
            this.key = "dsgl-application-portal-debug-tint"
            style = {
    ...
    override fun render(
        ...
        bounds = Rect(0, 0, viewportWidth, viewportHeight)
        val tintEnabled =
            DomainSurfaceDebugVisualization.enabled && isTintEnabled(ScreenDomainSurfaces.ApplicationPortal)
        if (tintEnabled) {
            debugTintNode.display = Display.Block
            debugTintNode.backgroundColor = DomainSurfaceDebugVisualization.applicationPortalFillColor
            debugTintNode.border = Border.all(1, DomainSurfaceDebugVisualization.applicationPortalBorderColor)
