...
internal class SystemInspectorPortalNode(
    ...
    private var floatingPanelDragUpdatedByDomInput: Boolean = false
    ...
    init {
        EventBus.run {
            this@SystemInspectorPortalNode.addEventListener(Events.MOUSEDOWN) { event: MouseDownEvent ->
                if (handleFloatingPanelMouseDown(event)) {
            ...
            this@SystemInspectorPortalNode.addEventListener(Events.MOUSEUP) { event: MouseUpEvent ->
                if (handleFloatingPanelMouseUp(event)) {
                    event.cancelled = true
                    return@addEventListener
                }
            ...
            this@SystemInspectorPortalNode.addEventListener(Events.DRAG) { event: MouseDragEvent ->
    ...
    private fun handleFloatingPanelMouseDown(event: MouseDownEvent): Boolean {
        ...
        val bodyRect = controller.portalContentRect()
        ...
        if (handled) {
            controller.onFloatingPanelPointerCaptureChanged(true)
        }
        return handled
    }

    private fun handleFloatingPanelDrag(mouseX: Int, mouseY: Int): Boolean {
        val handled =
            floatingPanel.handleMouseMove(
                ...
                controller.onFloatingPanelRectChanged(rect, lastViewportWidth, lastViewportHeight)
            }
        if (handled) {
            floatingPanelDragUpdatedByDomInput = true
            controller.onFloatingPanelPointerCaptureChanged(true)
        }
        return handled
    }

    private fun handleFloatingPanelMouseUp(event: MouseUpEvent): Boolean {
        val handled =
        ...
        if (handled) {
            floatingPanelDragUpdatedByDomInput = false
            controller.onFloatingPanelPointerCaptureChanged(false)
        }
        return handled
    }

    internal fun consumeFloatingPanelDomDragUpdate(): Boolean {
        val consumed = floatingPanelDragUpdatedByDomInput
        floatingPanelDragUpdatedByDomInput = false
        return consumed
    }
    ...
    fun syncInputBounds(viewportWidth: Int, viewportHeight: Int) {
        ...
        bounds = resolveInputBounds(viewportRect, controller.floatingPanelRect())
    }
    ...
    private fun resolveInputBounds(viewportRect: Rect, panelRect: Rect?): Rect {
        if (controller.blocksUnderlyingInput() || floatingPanel.isDragging() || minimizedChipDragSession != null) {
            return viewportRect
        }
    ...
    private fun resolveRenderedDropdownInputBounds(): Rect? {
        val dropdowns = controller.portalStyleEditorDropdowns()
    ...
    private fun renderExpanded(ctx: UiMeasureContext, snapshot: InspectorDomSnapshot, viewportRect: Rect) {
        ...
        val styleRows = controller.portalStyleEditorRows()
        ...
        renderTooltip(
            ...
            controller.portalVariableTooltip(),
            0xEE141A22.toInt(),
            0xCC60758F.toInt(),
        )
        renderTooltip(
            ...
            controller.portalCursorTooltip(),
            0xDD11151A.toInt(),
            0xCC3F4A57.toInt(),
        )
    }
    ...
    private fun renderExpandedChrome(scope: UiScope, ctx: UiMeasureContext, panelRect: Rect) {
        val pickRect =
            controller.portalPickToggleBounds()
                ?: Rect(panelRect.x + panelRect.width - 264, panelRect.y + 8, 160, 36)
        val minimizeRect =
            controller.portalMinimizeBounds()
    ...
    private fun renderHighlights(scope: UiScope, ctx: UiMeasureContext) {
        controller.portalSelectedHighlight()?.let { highlight ->
            renderHighlightRect(
                scope,
                ctx,
        ...
        controller.portalHoveredHighlight()?.let { highlight ->
            renderHighlightRect(
                scope,
                ctx,
    ...
    private fun renderStyleEditorStringInput(
        ...
        input.fontSize = 18
        input.onInput = {
            controller.portalApplyLiteralOverride(row.property, it.value)
        }
        input.onValueChange = {
            controller.portalApplyLiteralOverride(row.property, it.value)
        }
    ...
    private fun renderStyleEditorNumericControls(
        ...
        row.inputRect?.let { rect ->
            ...
            input.fontSize = 18
            input.onInput = {
                controller.portalApplyNumericOverride(row.property, it.value, row.unitValue)
            }
            input.onValueChange = {
                controller.portalApplyNumericOverride(row.property, it.value, row.unitValue)
            }
    ...
    private fun buildSystemOwnedSelectControl(
        ...
        val open = DomainPortalServices.isSelectOpenFor(key)
        val selectNode =
            scope.select(
                props = {
                    this.key = key
    ...
    private fun renderStyleEditorFooterActions(scope: UiScope, ctx: UiMeasureContext, bodyScrollY: Int) {
        val resetRect = controller.portalStyleEditorResetRect()
        ...
        val clearRect = controller.portalStyleEditorClearRect()
    ...
    private fun isDomOwnedInteractionTarget(target: DOMNode?): Boolean {
        ...
        while (current != null && current !== this) {
            ...
            if (nodeKey?.startsWith("dsgl-floating-panel-") == true) {
                return true
            }
