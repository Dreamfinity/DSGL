...
fun <S, A> UiScope.useReducer(
    ...
    return requireHookOwnerWindow().useReducer(initialState = initialState, reducer = reducer)
}

internal fun <S, A> DsglWindow.useReducer(
    initialState: S,
    reducer: (S, A) -> S
): Pair<S, (A) -> Unit> {
    return createReducerHookPair(
        window = this,
...
internal fun <S, A> createReducerHookPair(
    ...
    val signature = HookSignatures.reducer(
        stateClass = reducerStateClassOf(initialState),
        initialWasNull = initialState == null
    )
    val resolved = runtime.resolveUnnamedTypedEntry(
...
private fun reducerStateClassOf(value: Any?): KClass<*>? {
    return value?.let { current -> current::class }
}
