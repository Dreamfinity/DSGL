...
import org.dreamfinity.dsgl.core.colorpicker.internal.ColorPickerPopupPortalNode
...
import org.dreamfinity.dsgl.core.portal.input.dispatchManualThenDomFallback
...
class SystemPortalHost(
    ...
    private val rootNode: SystemPortalRootNode = SystemPortalRootNode()
    private val inspectorEntry: SystemPortalEntry = InspectorPortalEntry(inspectorController)
    private val colorPickerEntry: ColorPickerPortalEntry = ColorPickerPortalEntry()
    private val colorPickerTransientEntry: SystemPortalEntry = ColorPickerTransientPortalEntry(colorPickerEntry)
    private val entryRegistry: SystemPortalEntryRegistry =
        SystemPortalEntryRegistry(
    ...
    private val portalEntries: List<SystemPortalEntryAdapter> =
        entryRegistry.allEntries().map(::SystemPortalEntryAdapter)
    private val transientOwnershipRegistry: SystemPortalTransientOwnershipRegistry =
        SystemPortalTransientOwnershipRegistry()
    ...
    private val tree: DomTree =
        DomTree(
            root = rootNode,
            styleScope = StyleApplicationScope.System,
        )
    private var frameContext: SystemPortalFrameContext =
        SystemPortalFrameContext(
            inspectedRoot = null,
    ...
    fun appendFloatingPortalCommands(
        measureContext: UiMeasureContext,
        viewportWidth: Int,
        viewportHeight: Int,
    ...
    internal fun debugEntryState(id: SystemPortalEntryId): SystemPortalEntryState? = entryRegistry.entry(id)?.state

    internal fun debugEntryNode(id: SystemPortalEntryId): DOMNode? = entryRegistry.entry(id)?.node

    internal fun debugRegisteredEntryIds(): List<SystemPortalEntryId> = entryRegistry.allEntries().map { it.state.id }
    ...
    internal fun debugMountedEntryIds(): List<SystemPortalEntryId> {
        val entriesByNode = portalEntries.associateBy { it.node }
        val mountedNodes =
            buildList {
                addAll(rootNode.mountedLaneNodes(SystemPortalLane.PanelContent))
                addAll(rootNode.mountedLaneNodes(SystemPortalLane.Transient))
            }
    ...
    internal fun resolveTransientSession(ownerToken: Any): SystemPortalTransientSession =
        transientOwnershipRegistry.resolve(ownerToken)

    internal fun resolveTransientSession(ownerToken: Any, cursorX: Int, cursorY: Int): SystemPortalTransientSession =
    ...
    internal fun debugSystemColorPickerPopupOwnerDomain(): ScreenDomainId? = colorPickerEntry.debugOwnerDomain()
    ...
    private fun reconcileMountedEntries() {
        val activeEntries =
            portalHost.entriesInPaintOrder().mapNotNull {
                (it as? SystemPortalEntryAdapter)?.systemEntry
            }
        val panelNodes =
            activeEntries
                .filter { it.state.lane == SystemPortalLane.PanelContent }
                .map { it.node }
        val transientNodes =
            activeEntries
                .filter { it.state.lane == SystemPortalLane.Transient }
                .map { it.node }
    ...
    private fun activeEntriesTopFirst(): List<SystemPortalEntry> =
        portalHost
            .entriesInInputOrder()
            .mapNotNull { (it as? SystemPortalEntryAdapter)?.systemEntry }

    private inline fun dispatchManualInput(handler: (SystemPortalEntry) -> Boolean): Boolean =
        activeEntriesTopFirst()
            .asSequence()
            .filter { entry -> !entry.participatesInDomInput() }
            .any(handler)

    private class InspectorPortalEntry(
        private val inspectorController: InspectorController,
    ) : SystemPortalEntry {
        override val state: SystemPortalEntryState =
            SystemPortalEntryState(
                id = SystemPortalEntryId.Inspector,
                order = 100,
                lane = SystemPortalLane.PanelContent,
            )
        ...
        override val node: SystemInspectorPortalNode =
            SystemInspectorPortalNode(
        ...
        override fun sync(frame: SystemPortalFrameContext) {
            ...
            inspectorController.setFloatingPanelAuthorityEnabled(state.active)
            if (!state.active) {
                state.panelState.hide()
                state.dragSession.end()
                floatingPanel.syncPanelRect(null)
                inspectorController.onFloatingPanelPointerCaptureChanged(false)
                return
            }
            if (inspectorController.panelState == InspectorPanelState.Expanded) {
                ...
                val panelRect = inspectorController.floatingExpandedPanelRect()
                if (panelRect != null) {
                    inspectorController.onFloatingPanelRectChanged(panelRect, viewportWidth, viewportHeight)
                    floatingPanel.syncPanelRect(inspectorController.floatingExpandedPanelRect())
                } else {
                    state.panelState.show()
                    floatingPanel.syncPanelRect(state.panelState.currentRectOrNull())
                }
                val dragUpdatedByDomInput = node.consumeFloatingPanelDomDragUpdate()
                if (!dragUpdatedByDomInput) {
                    floatingPanel.handleMouseMove(
                        ...
                        inspectorController.onFloatingPanelRectChanged(rect, viewportWidth, viewportHeight)
                    }
                }
            } else {
                state.panelState.hide()
    ...
    private class ColorPickerPortalEntry :
        ...
        override val state: SystemPortalEntryState =
            SystemPortalEntryState(
                id = SystemPortalEntryId.ColorPickerPopup,
                order = 200,
                lane = SystemPortalLane.PanelContent,
            )
        ...
        override val node: ColorPickerPopupPortalNode = popupMount.node
        ...
        override fun sync(frame: SystemPortalFrameContext) {
            ...
            popupMount.floatingPanel.configure(
                ...
                style =
                    popupMount.popupEngine
                        ...
                        ?.let { toFloatingPanelStyle(it) }
                        ?: FloatingPanelStyle(),
                onClose = ::close,
            )
            val panelRect = popupMount.popupEngine.debugPanelRect(popupMount.ownerToken)
            if (panelRect != null) {
                popupMount.floatingPanel.syncPanelRect(panelRect)
            } else {
                state.panelState.show()
                popupMount.floatingPanel.syncPanelRect(state.panelState.currentRectOrNull())
            }
            if (popupMount.floatingPanel.handleMouseMove(
                    mouseX = frame.cursorX,
                    mouseY = frame.cursorY,
                    viewportWidth = viewportWidth,
                    viewportHeight = viewportHeight,
        ...
        override fun handleMouseDown(mouseX: Int, mouseY: Int, button: MouseButton): Boolean {
            ...
            if (popupMount.floatingPanel.handleMouseDown(mouseX, mouseY, button)) {
                return true
            }
        ...
        override fun handleMouseUp(mouseX: Int, mouseY: Int, button: MouseButton): Boolean {
            ...
            if (popupMount.floatingPanel.handleMouseUp(
                    mouseX = mouseX,
                    mouseY = mouseY,
                    button = button,
                    viewportWidth = viewportWidth,
        ...
        private fun shouldRouteSystemTextInputKeyDownToDom(): Boolean {
            if (popupMount.popupEngine.debugOwnerDomain(popupMount.ownerToken) != ScreenDomainId.System) return false
        ...
        fun transientPortalNode(): DOMNode = popupMount.transientNode
        ...
        fun debugHeaderRect(): Rect? = popupMount.floatingPanel.headerRect()

        fun debugCloseRect(): Rect? = popupMount.floatingPanel.closeRect()
        ...
        fun debugOwnerDomain(): ScreenDomainId? = popupMount.popupEngine.debugOwnerDomain(popupMount.ownerToken)
    }

    private class ColorPickerTransientPortalEntry(
        private val panelEntry: ColorPickerPortalEntry,
    ) : SystemPortalEntry {
        override val state: SystemPortalEntryState =
            SystemPortalEntryState(
                id = SystemPortalEntryId.ColorPickerTransient,
                order = 210,
                lane = SystemPortalLane.Transient,
            )
        override val node: DOMNode = panelEntry.transientPortalNode()
    ...
    private companion object {
        private fun inspectorPanelStyle(): FloatingPanelStyle =
            FloatingPanelStyle(
                headerHeight = 52,
                panelPadding = 6,
                resizeHandleSize = 8,
        ...
        private fun toFloatingPanelStyle(style: ColorPickerStyle): FloatingPanelStyle =
            FloatingPanelStyle(
